...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games AS a ON teams.team_id=a.winner_id FULL JOIN games AS b ON teams.team_id=b.opponent_id WHERE (a.year=2014 AND a.round='Eighth-Final') OR (b.year=2014 AND b.round='Eighth-Final') ORDER BY name")"
